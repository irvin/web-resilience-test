...
- 新增 Header 檢查機制（`cf-ray`、`x-amz-cf-pop`、`x-served-by`、`x-azure-ref`、`x-msedge-ref`）
- 未命中 `TPE` 時仍將掃到的 cloud header 寫入 `cloud_provider`（供事後分析 PoP，不影響分類）
- 新增 LACeS Anycast Census API 整合（header 失敗後、RTT 之前；見 `LACeS.zh-TW.md`）
...
- **預設（非 headless）** 失敗時重試：
  1. 原 URL（非 headless）
  2. `www.` 前綴（非 headless）
- **`--headless true`** 失敗時重試（單站載入通常較快，適合大批次）：
  1. 原 URL（headless）
  2. 原 URL（非 headless）
  3. `www.` 前綴（headless）
  4. `www.` 前綴（非 headless）
...
**D. IPinfo 與 LACeS 快取機制**
- 實作 IPinfo API 結果快取
- 實作 LACeS Anycast Census API 結果快取（`.cache/laces/`，以 `mapped_prefix` 或 IP 為鍵）
- 使用 MD5 hash 作為快取檔名
- 快取目錄結構化（`.cache/ipinfo/`、`.cache/laces/`）
- 預設 24 小時快取有效期
...
**G. Playwright 執行環境（2026-06）**
- 新增 `assertPlaywrightReady()`：單站與 batch 啟動前確認 Chromium 可啟動，避免大量無效 `_error`
- `--dns` 同步套用至 Playwright（Chromium `--dns-server`），使頁面載入與 ipinfo/LACeS 解析一致
- **預設改為非 headless**（較貼近一般使用者開啟瀏覽器；詳見 README）
- preflight 抓取 headed 預設 User-Agent，headless 與 headed 測試共用（取代硬編碼 Chrome 120）
...
- 錯誤日誌結構化（`_error/` 目錄）
- 批次摘要報告生成
- 全量測試前執行 Playwright preflight（`assertPlaywrightReady`）
...
**核心檔案：**
- **`report/index.md` / `report/en.md`**（596 行）：研究報告主文（初版 2026-03-23，最後更新 2026-05-06）
...
#### 2026年6月（Playwright 測試行為與方法論）

**2026-06-27**
- ✅ Playwright preflight（`assertPlaywrightReady`）：Chromium 未安裝或無法啟動時立即失敗，改善 `errorReason` 訊息
- ✅ `--dns` 同步至 Playwright（`--dns-server`），頁面載入與 ipinfo 解析共用 resolver
- ✅ 未命中 `TPE` 時仍記錄 `x-azure-ref` 等 cloud header 至 `cloud_provider`
- ✅ 預設改為非 headless；`--headless true` 四段重試（原 URL headless → 非 headless → www headless → www 非 headless）
- ✅ preflight 抓取 headed 預設 UA，所有模式共用（移除硬編碼 Chrome 120）
- ✅ README：補充 headless 速度優點、重試流程與非 headless 預設之設計說明（不宣稱未驗證的 PoP 路由結論）
...
3. **錯誤處理與穩定性**
   - 依模式區分的頁面載入重試策略（預設非 headless / `--headless true` 四段）
   - Playwright 啟動前 preflight 檢查
   - 詳細的錯誤日誌
   - Debug 模式
   - 超時處理改進
...
**重點：**
- 正式研究報告撰寫與發布（`report/index.md` / `report/en.md`）
- 統計圖表自動生成（SVG + PNG）
...
- **研究報告源碼：** `report/index.md` / `report/en.md`（建置後發布於 `report` branch worktree）
