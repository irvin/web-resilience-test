...
接著，本工具會整理頁面載入時蒐集的所有資源請求，先排除 `blob:` 等無法解析，或符合廣告過濾規則及手動指定清單中的 request。接著去除重複，同網站對同一 hostname 的多筆請求只保留一項，構成一組「網站－hostname」觀測標的。
...
[`no-global-connection-check.js`](https://github.com/irvin/web-resilience-test/blob/main_w_tw_result/no-global-connection-check.js) 用在單一網站的測試，流程如下：
  ...
  4. 清理 request 資料
     - 針對個別網站的 request 資訊，進行以下清理：
       ...
       - 排除手動指定清單中列出的 hostname。本研究僅指定 `fonts.gstatic.com` 網頁字型，因其載入失敗時應不影響網站核心功能。此清單機制亦可供後續研究納入其他排除項目
...
2,179 個完成測試的網站，共記錄 262,926 筆原始 HTTP request；經過上述過濾，並於各網站內依 hostname 去除重複後，產生 19,046 筆不重複的「網站－hostname」待分類觀測標的。所有觀測網站中共有 1,243 個網站的 3,640 筆觀測標的進入 RTT fallback 判定，涵蓋 976 個不同 hostname；細節如下表。
...
若將門檻改為 10 ms 或 20 ms，僅影響 32 個網站的分類：10 ms 時有 27 站由雲端依賴型改為境外依賴型；20 ms 時有 5 站由境外依賴型改為雲端依賴型。本地型網站數不受影響。

| RTT 門檻 | 境外依賴型  | 雲端依賴型    | 本地型        | 判定變更網站數 |
|----------|-------------|---------------|---------------|----------------|
...
本項比較中，「國際公有雲」包含本研究整理的跨國公有雲與 CDN 供應商，「其他／本地」則為其餘本地供應商。網站只要至少有一筆觀測標的請求屬於該分組，即計入對應類別：
...
將具有 IPinfo 提供之供應商資訊的 18,969 筆不重複「網站－hostname」觀測標的，根據 ASN organization 資料正規化進行統計排序，可發現網站依賴資源高度集中於大型供應商。其中超過 5% 的來源有 Google、Cloudflare、Amazon、中華電信及 Facebook。Google 的觀測占比最高，達 39.7%，Cloudflare 占 16.4%，Amazon 占 10.4%。
...
綜合上述研究限制，我們建議後續可進一步探討下述方向，以建立更完整的數位服務韌性評估框架：
   ...
   - 建立服務關鍵性框架，依政府、金融、通訊、影音串流、新聞、電商、社群、搜尋引擎等服務類別分類，並依安全、經濟與社會重要性加權，進行分類與比較其類型間的韌性差異，並計算整體韌性指數。
