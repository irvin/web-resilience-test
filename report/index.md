...
RTT 是位置判定的最後一層：當 IPinfo、response header 與 LACeS 皆無法確認公有雲資源位置時，才進入 RTT 判定。測試工具將對該資源 `ping` 五次，取最小 RTT；若小於 15 ms，判定為台灣境內的國際公有雲節點。

2,179 個完成測試的網站，共記錄 262,926 筆原始 HTTP request；排除廣告等項目，並於各網站內依 hostname 去除重複後，剩 19,046 筆待判定資源。其中 1,243 站進入 RTT 判定，涵蓋 976 個不同 hostname、共 3,640 筆觀測；細節如下表。

| 指標             | 數量    | 佔比及母體            |
|:-----------------|--------:|-----------------------|
| 完成測試網站     |   2,179 |                       |
| 含 RTT 判定網站  |   1,243 | 57.0%（完成測試網站） |
| 待判定資源       |  19,046 |                       |
| 進入 RTT 判定    |   3,640 | 19.1%（待判定資源）   |
| 成功取得 RTT     |   3,064 | 84.2%（進入 RTT 判定）|
| 最小 RTT < 15 ms |   2,394 | 78.1%（成功 RTT）     |
| 最小 RTT ≥ 15 ms |     670 | 21.9%（成功 RTT）     |
